...
class CityscapesCorruptionsDataset_clips(CustomDataset_cityscape_clips):
    ...
    def __init__(
        ...
        if corruption:
            if img_dir and osp.basename(osp.normpath(img_dir)) != corruption:
            ...
            for img in mmcv.scandir(img_dir, img_suffix, recursive=True):
                ...
                if ann_dir is not None:
                    if img.endswith(img_suffix):
                        seg_map = img[: -len(img_suffix)] + seg_map_suffix
                        if not osp.exists(osp.join(ann_dir, seg_map)):
                            seg_map = None
                img_info['ann'] = dict(seg_map=seg_map)
                img_infos.append(img_info)
    ...
    def _build_sequence_index(self):
        ...
        if missing_dirs:
            print_log(
                f'Found {len(missing_dirs)} image folders missing in Cityscapes corruptions',
