# dataset settings
dataset_type = 'ApolloScapeDataset_clips'
data_root = '/home/wangcl/data/open_video_DGSS/ApolloScape'
img_norm_cfg = dict(
    mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], to_rgb=True)
crop_size = (480, 480)
train_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(type='LoadAnnotations', reduce_zero_label=False),
    dict(type='Resize', img_scale=(853, 480), ratio_range=(0.5, 2.0), process_clips=True),
    dict(type='RandomCrop_clips', crop_size=crop_size, cat_max_ratio=0.75),
    dict(type='RandomFlip_clips', prob=0.5),
    dict(type='PhotoMetricDistortion_clips'),
    dict(type='Normalize_clips', **img_norm_cfg),
    dict(type='Pad_clips', size=crop_size, pad_val=0, seg_pad_val=255),
    dict(type='DefaultFormatBundle_clips'),
    dict(type='Collect', keys=['img', 'gt_semantic_seg']),
]
test_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(
        type='MultiScaleFlipAug',
        img_scale=(853, 480),
        flip=False,
        transforms=[
            dict(type='AlignedResize_clips', keep_ratio=True, size_divisor=32),
            dict(type='RandomFlip_clips'),
            dict(type='Normalize_clips', **img_norm_cfg),
            dict(type='ImageToTensor_clips', keys=['img']),
            dict(type='Collect', keys=['img']),
        ])
]
data = dict(
    samples_per_gpu=4,
    workers_per_gpu=4,
    train=dict(
        type='RepeatDataset',
        times=50,
        dataset=dict(
            type=dataset_type,
            data_root=data_root,
            img_dir='train/ColorImage',
            ann_dir='train/15Label',
            img_suffix='.jpg',
            seg_map_suffix='_bin.png',
            split='train',
            pipeline=train_pipeline,
            dilation=[-9, -6, -3],
            flip_video=True,
            group_by_camera=True)),
    val=dict(
        type=dataset_type,
        data_root=data_root,
        img_dir='val/ColorImage',
        ann_dir='val/15Label',
        img_suffix='.jpg',
        seg_map_suffix='_bin.png',
        split='val',
        pipeline=test_pipeline,
        dilation=[-9, -6, -3],
        flip_video=False,
        group_by_camera=True),
    test=dict(
        type=dataset_type,
        data_root=data_root,
        img_dir='val/ColorImage',
        ann_dir='val/15Label',
        img_suffix='.jpg',
        seg_map_suffix='_bin.png',
        split='val',
        pipeline=test_pipeline,
        dilation=[-9, -6, -3],
        flip_video=False,
        group_by_camera=True,
        mamba_mode=False))
